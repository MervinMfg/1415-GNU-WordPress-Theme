...
GNU.main = {
	...
	socialSliderInit: function () {
		var self, carousel, instagramUsername, instagramPosts, facebookUsername, facebookPosts;
		self = this;
		// Check if both facebook and insta have loaded, and if so render the carousel
		function socialLoadComplete() {
			if(instagramPosts.length > 0 && facebookPosts.length > 1) {
				i = 0; // used for incrimenting through the dom elements
				while(instagramPosts.length > 0 && facebookPosts.length > 1) {
					$('.social-slider .social-list').append(instagramPosts[0]);
					instagramPosts.shift();
					$('.social-slider .social-list').append('<div class="social-item facebook">' + facebookPosts[0] + facebookPosts[1] + '</div>');
					facebookPosts.shift();
					facebookPosts.shift();
				...
				// set up owl carousel
				carousel = $(".social-slider .owl-carousel").owlCarousel({
					items: 8,
					responsive: false,
					autoWidth: true,
					dots: false,
		...
		// INSTAGRAM POSTS
		instagramUsername = $('.social-slider').attr('data-instagram');
		instagramPosts = new Array();
		$.ajax({
			dataType: "json",
			url: '/feeds/instagram/?username=' + instagramUsername + '&limit=4',
			success: function (photosJSON) {
				...
				for (var i = 0; i < photosData.length; i++) {
					...
					postItem = '<div class="social-item instagram"><a href="' + photoData.link + '" target="_blank" class="owl-lazy" data-src="' + photoData.images.low_resolution.url + '" data-caption="' + photoData.caption.text + '"><div class="item-icon"><div class="icon"></div></div></a></div>';
					instagramPosts.push(postItem);
				}
				// randomize/shuffle instagram posts with underscore.js
				instagramPosts = _.shuffle(instagramPosts);
				socialLoadComplete();
			}
		});
		// FACEBOOK POSTS
		facebookUsername = $('.social-slider').attr('data-facebook');
		facebookPosts = new Array();
		$.ajax({
			dataType: "json",
			url: '/feeds/facebook/?username=' + facebookUsername + '&limit=16',
			success: function (postsJSON) {
				...
				for (var i = 0; i < postsData.length; i++) {
					...
					if (postData.type != "status") {
						...
						postItem = '<a href="' + postData.link + '" target="_blank"><div class="item-icon"><div class="icon"></div></div><div class="item-photo"><img src="' + self.config.wpImgPath + 'square.gif" alt="" class="lazy" data-src="' + postImage + '" /></div><div class="item-copy"><p class="fb-page">' + postData.from.name + ' on Facebook</p><p class="fb-date">' + postDate + '</p><p class="fb-excerpt">' + postMessage + '</p></div></a>';
						facebookPosts.push(postItem);
					}
				}
				socialLoadComplete();
			}
